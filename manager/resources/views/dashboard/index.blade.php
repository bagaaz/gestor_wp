...
@section('title', 'Dashboard - WP Docker Manager')
@section('heading', 'Dashboard')

@section('actions')
    <a href="{{ route('sites.create') }}" class="inline-flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900 mt-1">{{ $stats['total_sites'] }}</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-green-600 mt-1">{{ $stats['active_sites'] }}</p>
                </div>
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900 mt-1">
                        {{ number_format($stats['total_disk'] / 1024 / 1024, 0) }} MB
                    </p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl font-bold mt-1 {{ $stats['is_running'] ? 'text-green-600' : 'text-red-600' }}">
                        {{ $stats['is_running'] ? 'Rodando' : 'Parado' }}
                    </p>
                </div>
                <div class="w-12 h-12 {{ $stats['is_running'] ? 'bg-green-100' : 'bg-red-100' }} rounded-xl flex items-center justify-center">
                    <i class="fab fa-docker {{ $stats['is_running'] ? 'text-green-600' : 'text-red-600' }} text-xl"></i>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                    ...
                    <span class="text-sm text-gray-500">{{ $sites->count() }} sites</span>
                </div>
                ...
                @if($sites->isEmpty())
                    <div class="p-12 text-center">
                        ...
                        <a href="{{ route('sites.create') }}" class="inline-flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700">
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-4">
                                        <div class="w-10 h-10 rounded-lg flex items-center justify-center
                                            {{ $site->status === 'active' ? 'bg-green-100' : 'bg-gray-100' }}">
                                            <i class="fab fa-wordpress text-lg
                                                {{ $site->status === 'active' ? 'text-green-600' : 'text-gray-400' }}"></i>
                                        </div>
                                        <div>
                                            <a href="{{ route('sites.show', $site) }}" class="text-sm font-semibold text-gray-900 hover:text-blue-600">
                                                {{ $site->name }}
                                            </a>
                                            <div class="flex items-center gap-3 mt-0.5">
                                                <a href="{{ $site->url }}" target="_blank" class="text-xs text-blue-500 hover:underline">
                                                    {{ $site->url }} <i class="fas fa-external-link-alt text-[10px]"></i>
                                                </a>
                                                <span class="text-xs text-gray-400">WP {{ $site->wp_version }}</span>
                                                <span class="text-xs text-gray-400">{{ $site->formatted_disk_usage }}</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium
                                            ...
                                            <span class="w-1.5 h-1.5 rounded-full
                                                {{ $site->status === 'active' ? 'bg-green-500' : 'bg-gray-400' }}"></span>
                ...
                <div class="divide-y divide-gray-100">
                    @foreach($containers as $container)
                        <div class="px-6 py-3 flex items-center justify-between">
                            <div class="flex items-center gap-2">
                                <span class="w-2 h-2 rounded-full {{ $container['running'] ? 'bg-green-500' : 'bg-red-500' }}"></span>
                                <span class="text-sm text-gray-700">{{ str_replace('wp-', '', $container['name']) }}</span>
                            </div>
                            <span class="text-xs px-2 py-0.5 rounded
                                {{ $container['running'] ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                    ...
                    @forelse($recentLogs as $log)
                        <div class="px-6 py-3">
                            <div class="flex items-start gap-2">
                                <i class="fas fa-{{ $log->action === 'created' ? 'plus-circle text-green-500' : ($log->action === 'removed' ? 'minus-circle text-red-500' : 'info-circle text-blue-500') }} mt-0.5"></i>
                                <div>
