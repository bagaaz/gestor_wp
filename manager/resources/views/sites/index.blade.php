...
@section('title', 'Sites - WP Docker Manager')
...
@section('content')
    @if($sites->isEmpty())
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-16 text-center">
            ...
            <a href="{{ route('sites.create') }}" class="inline-flex items-center gap-2 bg-blue-600 text-white px-6 py-3 rounded-lg font-medium hover:bg-blue-700">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
                    <div class="p-6">
                        <div class="flex items-start justify-between mb-4">
                            <div class="flex items-center gap-3">
                                <div class="w-12 h-12 rounded-xl flex items-center justify-center
                                    {{ $site->status === 'active' ? 'bg-blue-100' : 'bg-gray-100' }}">
                                    <i class="fab fa-wordpress text-2xl
                                        {{ $site->status === 'active' ? 'text-blue-600' : 'text-gray-400' }}"></i>
                                </div>
                                <div>
                        ...
                        <div class="space-y-2 mb-4 text-sm">
                            <div class="flex items-center gap-2 text-gray-600">
                                ...
                                <a href="{{ $site->url }}" target="_blank" class="text-blue-600 hover:underline truncate">{{ $site->url }}</a>
                            </div>
                            <div class="flex items-center gap-2 text-gray-600">
                    ...
                    <div class="px-6 py-3 bg-gray-50 border-t border-gray-100 rounded-b-xl flex items-center justify-between">
                        ...
                        <form action="{{ route('sites.destroy', $site) }}" method="POST"
                              onsubmit="return confirm('Tem certeza que deseja remover o site {{ $site->name }}?')">
                            @csrf
                            @method('DELETE')
